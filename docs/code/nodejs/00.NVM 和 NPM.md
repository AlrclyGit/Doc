...
> node 版本管理工具还有一个是 TJ 大神的 n 命令，n 命令是作为一个 node 的模块而存在，而 nvm 是一个独立于 `node/npm` 的外部 shell 脚本，因此 n 命令相比 nvm 更加局限。由于 npm 全局安装的模块路径均为 `/usr/local/lib/node_modules`，当使用 n 切换不同的 node 版本时，实际上会共用全局的 `node/npm` 目录。因此不能很好地满足『按不同 node 版本使用不同全局 node 模块』的需求。同时 n 命令也无法管理它本身依赖的 node 版本。
...
> 命令中的 `v0.39.7` 是示例版本号，安装前建议到 [nvm 官方 Releases](https://github.com/nvm-sh/nvm/releases) 查看最新版本号并替换。

如果是 Mac 上安装报错，很可能是没有安装 Xcode 命令行工具（Command Line Tools），可以运行以下命令安装。
...
nvm install 4.2 #遵守语义化版本命名规则，安装 4.2.x 系列的最新版本
nvm install --lts #安装最新 LTS 版本 Node

nvm uninstall 4.2.2 #删除指定版本（若该版本正在使用中，需先切换到其他版本，否则会报错）

nvm list #查看本地 Node 版本列表
nvm current #查看当前使用的 Node 版本
nvm which 4.2.2 #确认某个版本 Node 的安装路径
...
NPM 是随同 Node.js 一起安装的包管理工具。由于新版 Node.js 已经集成了 npm，因此在安装 Node.js 时，npm 也会一并安装好。可以通过输入 `npm -v` 来验证是否安装成功。
...
npm cache clean --force #强制清空 npm 本地缓存（自 npm 5 起必须加 --force）
```
...
- dependencies - 依赖包列表。如果依赖包没有安装，npm 会自动将依赖包安装在 `node_modules` 目录下。
